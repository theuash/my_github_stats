...
const createTextNode = ({
  ...
  const animClass = animationType === "slide" ? "stagger-slide" : "stagger";
  ...
  `
    : "";
  return `
    <g class="${animClass}" style="animation-delay: ${staggerDelay}ms" transform="translate(25, 0)">
      ${iconSvg}
      <text class="stat ${
        bold ? " bold" : "not_bold"
...
const getStyles = ({
  ...
  return `
    ...
    .rank-circle-pulse {
      ...
      transform-origin: -10px 8px;
      transform: rotate(-90deg);
      animation: rankAnimation 1s forwards ease-in-out, pulseRingAnimation 2s 1.5s ease-in-out infinite;
    }
    .greeting-text {
    ...
    .greeting-wave {
      animation: waveAnimation 2.5s 0.5s ease-in-out infinite;
      transform-origin: 70% 70%;
      display: inline-block;
    }
    .header-glow {
      animation: fadeInAnimation 0.8s ease-in-out forwards, glowAnimation 3s 1s ease-in-out infinite;
    }
  ...
  */
  ...
  const rankCircle = hide_rank
    ...
    : `<g data-testid="rank-circle"
        ...
        <circle class="${animation_type === "pulse" ? "rank-circle-pulse" : "rank-circle"}" cx="-10" cy="8" r="40" />
        <g class="rank-text">
  ...
  const greeting = hide_title
    ? ""
    : `<text x="25" y="-5" class="greeting-text stagger" style="animation-delay: 100ms">
        Hey, <tspan font-weight="600">${encodeHTML(name)}</tspan>!
        <tspan class="greeting-wave">👋</tspan>
      </text>`;
